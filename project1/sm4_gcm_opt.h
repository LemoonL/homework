...
class sm4_gcm_opt {
public:
    sm4_gcm_opt(const uint8_t key[16], const uint8_t* iv, size_t iv_len);
...
private:
    ...
    inline void gmul(uint8_t X[16], const uint8_t Y[16]);
    inline void xor_block(uint8_t out[16], const uint8_t in[16]);
